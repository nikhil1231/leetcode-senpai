...
def test_expanded_corpus_inventory_and_content():
    ...
    assert Counter(e['mode'] for e in entries) == {'break': 16, 'trace': 29, 'fill': 44, 'approach': 49}
    assert sum(e['variants'] for e in entries) == 34
    assert {e['topic'] for e in entries} == set(neetcode150.CATEGORY_ORDER)
    assert len(practice_bank.CHOICES) == 85
...
assert len(_CHECKED_FILLS) == 25 and _CHECKED_FILLS[0] == 'fill-three-sum-skip'
