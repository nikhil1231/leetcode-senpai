...
(function () {
  ...
  function forecast(fc) {
    ...
    const cells = fc.counts.map((c, i) => {
      ...
      return `<div class="fc-cell" title="${label}: ${c} due"
        style="background:${c ? `rgba(124,134,255,${intensity.toFixed(2)})` : "var(--track)"}">
  ...
  function radar(series, size = 260) {
    ...
    const poly = (key, color, fill) => {
      ...
      return `<polygon points="${p}" fill="${fill}" stroke="${color}" stroke-width="2" stroke-linejoin="round"/>`;
    };
    const labels = series.map((s, i) => {
      ...
      return `<text x="${x.toFixed(1)}" y="${y.toFixed(1)}" text-anchor="middle" dominant-baseline="middle"
        font-size="9.5" fill="var(--muted)" font-weight="500">${esc(short)}</text>`;
    }).join("");
    return `<svg viewBox="0 0 ${size} ${size}" class="radar" role="img" aria-label="Mastery radar">
      ${ring(0.33)}${ring(0.66)}${ring(1)}
      ${poly("mastery_past", "var(--muted)", "rgba(118,127,149,0.14)")}
      ${poly("mastery", "var(--accent)", "rgba(124,134,255,0.26)")}
      ${labels}</svg>
  ...
  function lines(seriesMap, opts = {}) {
    ...
    const paths = names.map((n) => {
      ...
      return `<polyline points="${pts.join(" ")}" fill="none" stroke="${colors[n] || "var(--accent)"}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>` +
        seriesMap[n].map((d) => `<circle cx="${xOf(d.week).toFixed(1)}" cy="${yOf(d[opts.key || "median_min"]).toFixed(1)}" r="3" fill="${colors[n] || "var(--accent)"}"/>`).join("");
    }).join("");
    ...
    return `<svg viewBox="0 0 ${W} ${H}" class="linechart">
      ...
      <text x="${pad}" y="14" font-size="9.5" fill="var(--muted)" font-weight="500">${esc(opts.yLabel || "min")} (max ${maxY.toFixed(0)})</text>
