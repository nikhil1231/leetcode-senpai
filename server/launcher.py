...
def bind_available(host, port):
    ...
    for candidate in range(port, 65536):
        ...
        try:
            ...
            sock.close()
            if exc.errno != errno.EADDRINUSE or candidate == 65535:
                raise
    raise RuntimeError("No available port")


def run_local(host="127.0.0.1", port=8000, reload=True):
    config = uvicorn.Config("server.main:app", host=host, port=port, reload=reload)
    with bind_available(host, port) as sock:
